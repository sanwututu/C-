...
void CRoleDtMgr::loadFile(const string& strPath)
{
	...
	if (infile)
	{
		...
		for (int i = 0; i < nCount; i++)
		{
			...
			infile >>pData->nID>>pData->nHp>>pData->nAck>>pData->strName>>pData->strPic;
			m_VecData.push_back(pData);
		}
	}
	infile.close();
...
void CNpcDtMgr::loadFile(const string& strPath)
{
	...
	if (infile)
	{
		...
		for (int i = 0; i < nCount; i++)
		{
			...
			infile >> pData->nID >> pData->nMapID >> pData->nRow  >> pData->nCol>> pData->strName >> pData->strPic;
			m_VecData.push_back(pData);
		}
	}
	infile.close();
...
void CGoodsDtMgr::loadFile(const string& strPath)
{
	...
	if (infile)
	{
		...
		for (int i = 0; i < nCount; i++)
		{
			...
			infile >> pData->nID >> pData->nNpcID >> pData->strName >> pData->strPrice;
			m_VecData.push_back(pData);
		}
	}
	infile.close();
